...
namespace CAmod.Items.Weapons
{
    public class BlazingRevenant : ModItem
    {
            ...
            Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            float curved = (float)Math.Pow(ratio, 0.75f);
            // 체력 비율을 0.75 지수로 변형한다

            // =============================
            // 기존 Round 제거 → 확률 기반 count 계산
            // =============================
            float raw = MathHelper.Lerp(3f, 9f, curved);
            // 3~9 사이 실수값이다

            int lower = (int)Math.Floor(raw);
            // 내림값이다

            float frac = raw - lower;
            // 소수점 부분이다

            int count = lower + (Main.rand.NextFloat() < frac ? 1 : 0);
            // 소수점 확률로 +1 한다
